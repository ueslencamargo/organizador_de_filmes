...
import PIL #Peguei todo só pra garantir.
import io #Peguei todo só pra garantir.
import urllib
...
conn = connect(
    ...
    password='sucata',  # Senha do banco de dados
...
def exibir_imagens():
    ...
    def carregar_imagens_na_janela(indice):
        ...
        def ao_clicar_na_imagem(indice):
            ...
            subprocess.run(["python", "05_tela_filme.py", str(tmdb), str(arquivo), str(trailer)])
    ...
    janela.state('zoomed')
    janela.title("A lagarta disse que iria voar e todos riram dela, menos as borboletas. Dê ouvidos para quem acredita em você! Muitas vezes vai ser só você mesmo!")
    largura_janela = 1250
    altura_janela = 720
    ...
    janela.configure(bg="#0b151f")  # Define a cor de fundo da janela como preto
    janela.overrideredirect(False)  # Remove a barra de título da janela

    frame_principal = tk.Frame(janela, bg="#0b151f")  # Cria um frame na janela principal
    frame_principal.pack(pady=10)  # Define o espaçamento do frame

    frame_botoes = tk.Frame(frame_principal, bg="#0b151f")  # Cria um frame para os botões
    ...
    frame_imagens = tk.Frame(frame_principal, bg="#0A121E")  # Cria um frame para exibir as imagens
    ...
    mundo = mundo.resize((25, 25))  # Ajuste o tamanho da imagem conforme necessário
    mundo = ImageTk.PhotoImage(mundo)

    frame_botoes_inferiores = tk.Frame(janela, bg="#0b151f")  # Cria um frame para os botões inferiores
    frame_botoes_inferiores.pack(side="top", pady=(0,10))  # Define o espaçamento do frame de botões inferiores

    botao_adicionar = tk.Button(frame_botoes_inferiores, text="Adicionar Filme", width=15,command=lambda: subprocess.run(["python", "06_cadastrar_filme.py", str(bd)]))  # Cria um botão para adicionar filmes
    botao_adicionar.pack(side="left", padx=10)  # Posiciona o botão na parte esquerda do frame

    botao_editar = tk.Button(frame_botoes_inferiores, text="Editar Filmes", width=15,command=lambda: subprocess.run(["python", "08_alterar.py", str(bd)]))  # Cria um botão para adicionar filmes
    botao_editar.pack(side="left", padx=30)  # Posiciona o botão na parte esquerda do frame

    texto_lateral = tk.Label(frame_botoes_inferiores, text=bd, fg="white", bg="#0b151f", font=("Calibri", 12, "italic"))
    texto_lateral.pack(side="left", padx=40)

    caixa_pesquisa = tk.Entry(frame_botoes_inferiores, bg="dark grey", fg="black", width=40, font=18)  # Cria uma caixa de texto para a pesquisa
    ...
    caixa_pesquisa.pack(side="left", padx=20)  # Posiciona a caixa de pesquisa no frame
    caixa_pesquisa.bind("<Return>", capturar_texto)  # Captura o texto quando a tecla Enter é pressionada

    texto_lateral = tk.Label(frame_botoes_inferiores, text=f"{nfilmes} filmes cadastrados", fg="white", bg="#0b151f", font=("Calibri", 12, "italic"))
    texto_lateral.pack(side="left",padx=60)

    botao_redondo = tk.Button(frame_botoes_inferiores, image=mundo, borderwidth=0, highlightthickness=0, bg="#0b151f", activebackground="#0b151f", command=lambda: amigo())
...
def amigo():
    ...
    bdamigo = simpledialog.askstring(":D", "Digite o nome de usuário:")
    subprocess.run(["python", "04_tela_inicial_amigo.py", str(bdamigo)])
